package com.games.problemset.sortings.cottagevillage;
...
public class Solution {
    ...
    static class Item {
        int pos;
        int side;
        static Item of(int pos, int side) {
            Item item = new Item();
            item.pos = pos;
            item.side = side;
            return item;
        ...
        public String toString() {
            return String.format("(%d : %d)", pos, side);
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        items.sort((a, b) -> Integer.compare(a.pos, b.pos));
        O.debug(S.stringi(items));
        int pos = 2;
        for (int i = 0; i < count - 1; i++) {
            float end = items.get(i).pos + items.get(i).side / 2f;
            float str = items.get(i + 1).pos - items.get(i + 1).side / 2f;
            O.debug(end + " " + str + " \n");
            if (str - end == side) pos += 1;
            if (str - end > side) pos += 2;
        }
        O.print(pos);
    }
