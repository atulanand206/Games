package com.games.problemset.sortings.diamondminer;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            int n = I.inputInt(br);
            List<Long> miners = new ArrayList<>();
            List<Long> mines = new ArrayList<>();
            for (int i = 0; i < 2 * n; i++) {
                long[] spec = I.inputLongArray(br);
                if (spec[0] == 0) miners.add(Math.abs(spec[1]));
                else mines.add(Math.abs(spec[0]));
            }
            double xs = 0;
            Collections.sort(miners);
            Collections.sort(mines);
            for (int i = 0; i < n; i++) {
                xs += Math.sqrt(miners.get(i) * miners.get(i) + mines.get(i) * mines.get(i));
            }
            double nxs = 0;
            Collections.reverse(mines);
            for (int i = 0; i < n; i++) {
                nxs += Math.sqrt(miners.get(i) * miners.get(i) + mines.get(i) * mines.get(i));
            }
            sb.append(S.string(Math.min(xs, nxs)));
        }
        O.print(sb);
    }
