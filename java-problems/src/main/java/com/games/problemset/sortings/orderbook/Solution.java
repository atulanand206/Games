package com.games.problemset.sortings.orderbook;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = spec[0], s = spec[1];
        StringBuilder sb = new StringBuilder();
        Map<Integer, Integer> buy = new TreeMap<>(Comparator.reverseOrder());
        Map<Integer, Integer> sell = new TreeMap<>();
        while (n-- > 0) {
            String[] st = I.inputStringArray(br);
            int p = Integer.parseInt(st[1]);
            int v = Integer.parseInt(st[2]);
            switch(st[0]) {
                case "B":
                    buy.merge(p, v, Integer::sum);
                    break;
                case "S": 
                    sell.merge(p, v, Integer::sum);
                    break;
            }
        }
        append(sell, s, 'S', sb);
    ...
    private static void append(Map<Integer, Integer> map, int s, char ch, StringBuilder sb) {
        List<Map.Entry<Integer, Integer>> set = new ArrayList<>(map.entrySet());
        set = set.subList(0, Math.min(map.size(), s));
        if (ch == 'S') Collections.reverse(set);
        for (int i = 0; i < set.size(); i++) 
            sb.append(ch).append(" ").append(set.get(i).getKey()).append(" ").append(set.get(i).getValue()).append("\n");
    }
