package com.games.problemset.greedy.nonsubstringsubsequence;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            int n = spec[0], q = spec[1];
            char[] chs = I.inputString(br).toCharArray();
            while (q-- > 0) {
                spec = I.inputIntArray(br);
                int l = spec[0] - 1, r = spec[1] - 1;
                boolean x = isPresent(chs, r + 1, chs.length - 1, chs[r]);
                boolean y = isPresent(chs, 0, l - 1, chs[l]);
    ...
    private static boolean isPresent(char[] chs, int s, int e, int k) {
        for (int i = s; i <= e; i++)
            if (chs[i] == k)
                return true;
        return false;
