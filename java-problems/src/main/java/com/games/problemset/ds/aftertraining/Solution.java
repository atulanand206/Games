package com.games.problemset.ds.aftertraining;
...
public class Solution {
    ...
    static class Item {
        ...
        static Item of(int indx, int qty) {
            Item it = new Item();
            it.indx = indx;
            it.qty = qty;
            return it;
        ...
        public String toString() {
            return String.format("%s -> %s", indx, qty);
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        PriorityQueue<Item> q = new PriorityQueue<>((a, b) -> {
            if (a.qty == b.qty) return Integer.compare(dist(m, a.indx), dist(m, b.indx));
        ...
        while (n-- > 0) {
            ...
            O.debug(S.string(q));
            Item entry = q.poll();
            sb.append(entry.indx + 1).append("\n");
            q.add(Item.of(entry.indx, entry.qty + 1));
        }
        O.print(sb);
    }

    private static int dist(int m, int x) {
        return Math.abs((m + 1) / 2 - x); 
    }
