package com.games.CodeForces.amrandmusic;
...
public class Solution {
    ...
    static class Item {
        ...
        public String toString() {
            return String.format("(%d=>%d)", indx, val);
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i =0 ; i < arr.length; i++) items.add(new Item(i + 1, arr[i]));
        items.sort(Comparator.comparingInt(Item::getVal));
        int sum = 0;
        List<Item> vals = new ArrayList<>();
        int i = 0;
        while (i < items.size() && sum + items.get(i).getVal() <= k) {
            sum += items.get(i).getVal();
            vals.add(items.get(i));
            i++;
        }
        StringBuilder sb = new StringBuilder();
        vals.sort(Comparator.comparingInt(Item::getIndx));
