package com.games.CodeForces.maximandbiology;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int len = I.inputInt(br);
        char[] s = I.inputString(br).toCharArray();
        int t = len - 4;
        int min = Integer.MAX_VALUE;    
        for (int i = 0; i <= t; i++) {
            int moves = move(s , i, i + 4);
            min = Math.min(min, moves);
        }
        O.print(min);
    }

    private static final char[] GENOME = new char[] {'A', 'C', 'T', 'G'};

    private static int move(char[] str, int s, int e) {
        int min = 0;
        for (int i = 0; i < 4; i++) {
            min += mindist(str[s + i], GENOME[i]);
        }
        return min;
    }

    private static int mindist(char ch1, char ch2) {
        int min = Math.abs((int)ch1 - (int)ch2);
        min = Math.min(min, Math.abs((int)ch1 + 26 - (int)ch2)); 
        min = Math.min(min, Math.abs((int)ch1 - 26 - (int)ch2));
        return min;
    }
