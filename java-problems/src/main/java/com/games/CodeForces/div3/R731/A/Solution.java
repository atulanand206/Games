package com.games.CodeForces.div3.R731.A;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            int[] f = I.inputIntArray(br);
            int xd = Math.abs(a[0] - b[0]);
            int yd = Math.abs(a[1] - b[1]);

            int x = (b[1] - f[1]) * (a[1] - f[1]);
            int y = (b[0] - f[0]) * (a[0] - f[0]);

            if (xd == 0) {
                int xi = ((x < 0) && (f[0] == a[0])) ? (yd + 2) : yd;
                sb.append(xi + "\n");
                continue;
            }
            if (yd == 0) {
                int yi = ((y < 0) && (f[1] == a[1])) ? (xd + 2) : xd;
                sb.append(yi + "\n");
                continue;
            }
