package com.games.CodeForces.vitaliyandpie;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[] chs = I.inputString(br).toLowerCase().toCharArray();
        boolean[] open = new boolean[t];
        for (int i = 0; i < chs.length; i++) {
            if (i % 2 == 0) map.merge(chs[i], 1, Integer::sum);
            else {
        ...
        O.debug(S.string(open));
        int z = 0;
        for (boolean b : open) if (!b) z++;
        O.print(z);
    }
