package com.games.CodeForces.santaclausandcandies;
...
public class Solution {
    ...
    private static StringBuilder greedy(int n) {
    ...
    private static StringBuilder dp(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            dp.add(set);
            for (int j = 1; j < i; j++) {
                if (!dp.get(j).contains(i - j)) {
                    if (dp.get(j).size() > dp.get(x).size()) {
                        x = j;
                    }
                }
            }
            Set<Integer> set1 = new HashSet<>(dp.get(x));
            set1.add(i - x);
            dp.set(i, set1);
        }
        O.debug(S.stringi(dp));
        Set<Integer> res = dp.get(n);
        res.remove(0);
        StringBuilder sb = new StringBuilder();
