package com.games.CodeForces.comparingstrings;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (new String(a).equals(new String(b))) {
            ...
            for (char ch : a) map.merge(ch, 1, Integer::sum);
            O.print(map.size() < a.length ? "YES": "NO");
            return;
        }
        ...
        for (int i = 0; i < d.length; i++)
            d[i] = (int)a[i] - (int)b[i];
        int z = 0;
        for (int i : d) if (i == 0) z++;
        if (z == d.length - 2) {
            int sum = 0;
            for (int i : d) sum += i;
            O.print(sum == 0 ? "YES" : "NO");
            return;
        }
        O.print("NO");
    }
