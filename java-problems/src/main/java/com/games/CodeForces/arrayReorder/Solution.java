package com.games.CodeForces.arrayReorder;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < arr.length; i++) b[i] = arr[arr.length - 1 - i];
            for (int i = 0; i < arr.length; i++) b[i] = findSeries(arr[i]);
            O.write(S.string(arr));
            O.write(S.string(b));
            Map<Integer, List<Integer>> map = new HashMap<>();
            for (int i = 0; i < b.length; i++) {
                List<Integer> elems = map.getOrDefault(b[i], new ArrayList<>());
                elems.add(i);
                map.put(b[i], elems);
            }
            int k = 0;
            int[] res = new int[n];
            List<Map.Entry<Integer, List<Integer>>> entries = map.entrySet().stream()
                    .sorted(Map.Entry.comparingByKey()).collect(Collectors.toList());
            Collections.reverse(entries);
            O.write(S.string(entries));
