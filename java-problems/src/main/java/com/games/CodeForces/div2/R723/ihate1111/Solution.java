package com.games.CodeForces.div2.R723.ihate1111;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < ones.length; i++)
            ones[i] = one(i + 2);
        O.debug(S.string(ones));
        boolean[] dp = new boolean[(int)(1E9) + 5];
        dp[0] = true;
        for (int i : ones) dp[i] = true;
    ...
    private static int one(int x) {
        return Integer.parseInt(new String(new char[x]).replaceAll("\0", "1"));
    }
