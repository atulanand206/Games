package com.games.CodeForces.div2.R724.B;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            List<String> subStrings = subString(s);
            boolean val = true;
            for (int i = 1; i < subStrings.size(); i++) {
                val = stringCompare(subStrings.get(i - 1), subStrings.get(i));
                if (!val) {
                    sb.append(next(subStrings.get(i - 1))).append("\n");
                    break;
                }
            }
            if (val) sb.append(next(subStrings.get(subStrings.size() - 1))).append("\n");
        }
        O.print(sb);
    }
    ...
    public static boolean stringCompare(String str1, String str2) {
        ...
        if (str1.length() == str2.length()) {
            ...
            int minus25 = 0;
            for (int j : diff) if (j == -25) minus25++;
    ...
    private static String next(String str) {
        ...
        return new String(new char[chars.length + 1]).replace("\0", "a");
    }
