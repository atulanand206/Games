package com.games.CodeForces.div2.R728.pleasantpairs;
...
public class Solution {
     ...
     * n = 8; 2n = 16 ai * aj = i + j
     * 
     * ai <= 2n = 16 ai * aj <= 4n^2 = 256
     * 
     * 2 8
     * 
     * i + j <= 2n
     * 
     * // 4n^2 <= 2n // 4n^2 - 2n <= 0 // 2n(2n - 1) <= 0 // n (n - 1 / 2) <= 0
     */
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < arr.length - 1; i++) {
                for (int j = i + 1; j < arr.length; j++) {
                    long y = Math.min(arr[i], arr[j]);
                    long z = 2 * n / y;
                    if (y > 1) {
                        
                        if (arr[i] <= z && arr[j] <= z) {
                            O.debug(String.format("%d : %d : %d : %d\n", arr[i], arr[j], arr[i] * arr[j], i + j + 2));
                            
                            if (arr[i] * arr[j] == i + j + 2)
                                x++;

                        } else {
                            O.debug(String.format("zzzzzz%d : %d : %d : %d\n", arr[i], arr[j], arr[i] * arr[j], i + j + 2));
                    
                        }
                    } else {
                        O.debug(String.format("k------%d : %d : %d : %d\n", arr[i], arr[j], arr[i] * arr[j], i + j + 2)); 
                        if (arr[i] * arr[j] == i + j + 2) x++;
                    }
                }
            }
            O.debug("\n");
