package com.games.CodeForces.div2.R727.C;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        O.debug(S.string(dp));
        List<Long> vacs = new ArrayList<>();
        for (long i : dp) if (i > x) vacs.add(i);
        long ans = vacs.size() + 1;
        Collections.sort(vacs);
        O.debug(S.string(vacs));
        // vacs = vacs.stream().map(m -> m / x).sorted().collect(Collectors.toList());
        // O.debug(S.string(vacs)); 
        for (long v : vacs) {
            long m  = v / x;
            if (v % x == 0) m--;
