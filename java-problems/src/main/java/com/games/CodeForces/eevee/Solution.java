package com.games.CodeForces.eevee;
...
public class Solution {
    ...
    private static final String[] NAMES = new String[]{"vaporeon", "jolteon", "flareon", "espeon", "umbreon", "leafeon", "glaceon", "sylveon"};
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<String> a = Arrays.stream(NAMES).collect(Collectors.toList());
        for (int i = 0; i < t; i++) {
            if (s[i] != '.') {
                for (int j = 0; j < a.size(); j++) {
                    if (j < a.size() && a.get(j).charAt(i)!=s[i]) {
                        a.remove(j);
                        j--;
                    }
                }
