package com.games.CodeForces.diversity;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (char ch : chs) map.merge(ch, 1, Integer::sum);
        int distinct = map.size();
        if (distinct >= t) {
            O.print(0);
            return;
        }
        int toChange = t - distinct;
        int avaiSwaps = chs.length - distinct;
        O.debug(toChange);
        O.debugNewLine();
        O.debug(avaiSwaps);
        O.debugNewLine();
        O.debug(distinct);
        O.debugNewLine();
        if (toChange < 0 || avaiSwaps < toChange) O.print("impossible");
        else O.print(toChange);
    }
