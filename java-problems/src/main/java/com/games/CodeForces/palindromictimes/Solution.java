package com.games.CodeForces.palindromictimes;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String str = I.inputString(br);
        Set<String> set2 = new TreeSet<>();
        for (int i = 0; i < 24; i++) 
            for (int j = 0; j < 60; j++) 
                if (isPalindrome(i, j)) 
                    set2.add(String.format("%02d:%02d", i, j));
        for (String st : set2) {
        ...
        O.print(set2.iterator().next());
    }

    private static boolean isPalindrome(int i, int j) {
        return new StringBuilder(String.format("%02d", i)).reverse().toString().trim()
                .equals(String.format("%02d", j).trim());
    }
