package com.games.CodeChef.LTIME97B.tictactoe;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            int r = spec[0], c = spec[1], k = spec[2];
            int[] aliceRow = new int[r]; Arrays.fill(aliceRow, 0);
            int[] bobRow = new int[r]; Arrays.fill(bobRow, 0);
            int[] aliceCol = new int[c]; Arrays.fill(aliceCol, 0);
            int[] bobCol = new int[c]; Arrays.fill(bobCol, 0);
            int tot = r * c;
            boolean fin = false;
            for (int m = 0; m < tot; m++) {
                spec = I.inputIntArray(br);
                if (fin) continue;
                int x = spec[0] - 1, y = spec[1] - 1;
                if (player == 'A') {
                    aliceRow[x]++;
                    aliceCol[y]++;
                    boolean alice = iswinner(aliceRow, aliceCol, x, y, k);
                    if (alice) {
                        O.debug(S.string(aliceRow));
                        O.debug(S.string(aliceCol));
                    ...
                    bobRow[x]++;
                    bobCol[y]++;
                    boolean bob = iswinner(bobRow, bobCol, x, y, k);
                    if (bob) {
                        O.debug(S.string(bobRow));
                        O.debug(S.string(bobCol));
    ...
    private static boolean iswinner(int[] row, int[] col, int x, int y, int k) {
        int c = 0;
        boolean r = false, cl = false;
        for (int i = Math.max(0, x - k); i < Math.min(row.length, x + k); i++) {
            if (row[i] >= k) c++;
            else c = 0;
            if (c >= k) {
                r = true;
        ...
        c = 0;
        for (int i = Math.max(0, y - k); i < Math.min(col.length, y + k); i++) {
            if (col[i] >= k) c++;
            else c = 0;
            if (c >= k) {
                cl = true;
