package com.games.CodeChef.START5B.flappybirds;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            int n = spec[0], H = spec[1];
            int[] xi = I.inputIntArray(br);
            int[] hi = I.inputIntArray(br);
            int moves = 0;
            boolean fail = false;
            int cx = 0, cy = H;
            O.debug(S.string(cy));
            ...
            for (int i = 0; i < n; i++) {
                while (i < hi.length && hi[i] < cy) {
                    i++;
                }
                if (i < hi.length) {
                    inc.add(i);
                    cy = hi[i];
                    O.debug(String.format("%d %d \n", xi[i], cy));
                }
            }
            O.debug(S.string(inc));
            cx = 0; cy = H;
            int tx = xi[xi.length - 1];
            int ty = hi[inc.get(inc.size() - 1)];
            ...
            if (!fail) sb.append(moves).append("\n");
        }
        O.print(sb);
    }
