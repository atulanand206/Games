package com.games.CodeChef.Jun21B.summerHeat;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            int xa = spec[0], xb = spec[1], Xa = spec[2], Xb = spec[3];
            int ans = 0;
            if (xa != 0) ans += Math.ceil(Xa / (double) xa);
            if (xb != 0)  ans += Math.ceil(Xb / (double) xb);
