package com.games.CodeChef.SNCKQL21.C;
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < arr.length; i++) {
                List<Integer> lst = mp.getOrDefault(arr[i], new ArrayList<>());
                lst.add(i);
                mp.put(arr[i], lst);
            }
            O.debug(S.string(mp));
            int k = 0;
            int[] c = new int[n];
            List<Integer> treeSet = new ArrayList<>(mp.keySet());
            Collections.sort(treeSet);
            for (int i : treeSet) {
                List<Integer> indxs = mp.get(i);
                for (int indx : indxs) {
                    if (i > k) {
                        c[indx] = k;
                        k++;
